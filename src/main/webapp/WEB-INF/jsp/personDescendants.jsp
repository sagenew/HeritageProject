...
<html>
  <head>
    <meta charset="UTF-8" />
    <title><spring:message code="label.personDescendantsHeader" /></title>
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/style.css"/>
  </head>
  <body>
  <jsp:useBean id="date" class="java.util.Date"/>
    <h1><spring:message code="label.personDescendantsHeader" /></h1>
    <c:choose>
        <c:when test="${descendants.isEmpty()}">
            <i><p style="font-size:120%; "><spring:message code="label.noDescendants" /></p></i>
        </c:when>
    <c:otherwise>
    <div>
      <table border="1">
        ...
        <c:forEach items="${descendants}" var ="person">
        <tr>
          <td>${person.id}</td>
