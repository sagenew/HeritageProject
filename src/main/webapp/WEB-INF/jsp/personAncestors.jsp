...
<html>
  <head>
    <meta charset="UTF-8" />
    <title><spring:message code="label.personAncestorsHeader" /></title>
    <link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/css/style.css"/>
  </head>
  <body>
  <jsp:useBean id="date" class="java.util.Date"/>
    <h1><spring:message code="label.personAncestorsHeader" /></h1>
    <c:choose>
        <c:when test="${ancestors.isEmpty()}">
            <i><p style="font-size:120%; "><spring:message code="label.noAncestors" /></p></i>
        </c:when>
    <c:otherwise>
    <div>
      <table border="1">
        ...
        <c:forEach items="${ancestors}" var ="person">
        <tr>
          <td>${person.id}</td>
